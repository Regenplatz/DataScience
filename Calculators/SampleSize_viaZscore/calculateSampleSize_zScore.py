...
class CalculateSampleSize:
    ...
    def __init__(self, population_size: int, ci: int, me: int, p: int = 50) -> None:
        """
        Initialize class variables.
        :param population_size: int, size of population (absolute number)
        :param ci: int, confidence interval in % (e.g. 95 for 95%)
        :param me: int, margin of error in % (e.g. 5 for 5%)
    ...
    def evaluate_zScore(self) -> float:
        """
        ...
        def evaluateClosestValue(lst: list, a: float) -> float:
            """
        ...
        def lookUpZScore(zScoreTable: pd.DataFrame, areaVal: float) -> float:
            """
            ...
            for idx, row in zScoreTable.iterrows():
                ...
                ## in case a value is available in several cells,
    ...
    def formulaUnknownHugePopulation(self) -> int:
        """
...
def main():
    ...
    classObject = CalculateSampleSize(N, ci, me, p)
    classObject.evaluate_zScore()
    print("Required Sample Size:", classObject.formulaStandard())

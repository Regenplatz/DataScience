...
class TrendingRules:
    ...
    def __init__(self, df_dataPoints, df_all, colName):
        """
        Initialize class variables.
        :param df_dataPoints: dataframe, containing the data of interest
        :param df_all: dataframe, containing all data to assess mean and +-1/2/3 std
        :param colName: String, column name containing data of interest to be processed / analyzed
        """
        self.df = df_dataPoints
        self.colName = colName
        self.data = self.df[self.colName]
        self.mean = np.mean(df_all[colName], axis=0)
        self.sigma = np.std(df_all[colName], axis=0)
